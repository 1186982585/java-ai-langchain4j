...
public class AppointmentTools {
    ...
    @Tool(name = "取消预约挂号", value = "根据参数，查询预约是否存在，如果存在则删除预约记录并返回取消预约成功，否则返回取消失败")
    ...
    @Tool(name = "更改预约挂号", value = "根据参数，查询预约是否存在，如果存在让用户输入更改后的时间、科室和医生，并让用户确认更改后的信息，用户确定后进行更改操作")
    public String updateAppointment(
        ...
        Appointment appointmentDB = appointmentService.getOne(appointment);
        if (appointmentDB != null) {
            appointmentDB.setDate(date);
            appointmentDB.setTime(time);
            appointmentDB.setDoctorName(doctorName);
